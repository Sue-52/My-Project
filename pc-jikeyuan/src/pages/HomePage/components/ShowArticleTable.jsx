...
import { Card, Table, Space, Button, Tag, message, Image } from "antd";
...
function ShowArticleTable() {
  ...
  const getData = async () => {
    try {
      ...
      message.error(error.response?.data?.message || "列表数据获取失败");
    }
  };
  ...
  const [dataSource, setDataSource] = useState(selector.results);
